...
class OraclePlanner(Planner):
    def __init__(self, traversable_colors, goal_color, room_or_object_goal, world_image_filename, env_to_coor, env_next_coords, env_to_grid, env_grid_resolution, env_world_array, env_render, name="Oracle"):
        ...
        print(world_image_filename)
        self.world_image_filename = world_image_filename
        self.full_size_semantic_gridmap = plt.imread(self.world_image_filename)
    ...
    def plan(self, obs):
        ...
        if self.actions_to_goal is None:
            traversable_array, _, _ = find_traversable_inds(self.full_size_semantic_gridmap, self.traversable_colors)
            goal_array, _, _ = find_goal_inds(self.full_size_semantic_gridmap, self.goal_color, self.room_or_object_goal)
    ...
    def plot_oracle_path(self):
        if self.plot_panels:
            ...
            path_array = np.zeros((self.full_size_semantic_gridmap.shape[0], self.full_size_semantic_gridmap.shape[1]))
            ...
            planner_array = np.zeros_like(self.full_size_semantic_gridmap)
